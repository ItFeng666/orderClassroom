	...
	<view class="w-picker-view">
		<picker-view v-if="fields=='year'" class="d-picker-view" :indicator-style="itemHeight" :value="pickVal" @change="handlerChange"
		 @pickstart="pickstart" @pickend="pickend">
			<picker-view-column>
				<view class="w-picker-item" v-for="(item,index) in range.years" :key="index">{{item === '全部' ? item : item + '年'}}</view>
			</picker-view-column>
		</picker-view>
		<picker-view v-if="fields=='month'" class="d-picker-view" :indicator-style="itemHeight" :value="pickVal" @change="handlerChange"
		 @pickstart="pickstart" @pickend="pickend">
			...
			<picker-view-column :style="{ visibility: checkObj.year === '全部' ? 'hidden' : '' }">
	...
	export default {
		...
		methods: {
			...
			getDval() {
				...
				if (value) {
					let flag = value.indexOf('全部') !== -1  ? true : this.checkValue(value);
					if (!flag) {
